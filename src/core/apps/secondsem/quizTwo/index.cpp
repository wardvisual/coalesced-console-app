/**
 * @file /src/core/apps/secondsem/quiztwo/index.cpp
 ...
 */
void quizTwo(std::string currentMenu) {
  /* Declaring variables. */
  ...
  reViewMainScreen(currentMenu, quizTwoHeading);
  ...
  if (userChoice == MENU_ITEM_1) {
    renderDaysConversion(currentMenu, quizTwoHeading);
  }

  if (userChoice == MENU_ITEM_2) {
    renderTimeConversion(currentMenu, quizTwoHeading);
  }
}
